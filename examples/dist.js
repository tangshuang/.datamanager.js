...
function setDataItem(source, requestId, data) {
  ...
  var snapshots = item.snapshots = item.snapshots || [];
  var time = Date.now();
  var dataItem = {
    time: time,
    data: data
  ...
  _createClass(DataManager, [{
    ...
    value: function _debug() {
      if (this.settings.debug) {
        ...
        (_console = console).log.apply(_console, [this.id].concat(args));
      }
    }
  }, {
    key: 'register',
    value: function register(datasource) {
      ...
      this.datasources[id] = (0, _lodash4.default)({}, source, { transformers: transformers });
    ...
    value: function unsubscribe(id, callback) {
      ...
      source.callbacks = callbacks.filter(function (item) {
        if (item.context === _this2.id) {
    ...
    value: function _addDep() {
      ...
      if (this._deps.find(function (item) {
        return item.id === id && isEqual(item.params, params) && item.target === target;
      })) {
        return false;
      }
    ...
    value: function autofree(funcs) {
      ...
      funcs.forEach(function (fun) {
        var deps = _this5._deps.filter(function (item) {
          return item.target === fun;
        });
        deps.forEach(function (dep) {
          _this5.unsubscribe(dep.id, dep.callback);
        });
        _this5._deps = _this5._deps.filter(function (item) {
          return item.target !== fun;
        });
      });
    }
    ...
    value: function get(id) {
      ...
      var item = store[requestId];
      var expires = this.settings.expires;
...
var ComponentA = function () {
  ...
  _createClass(ComponentA, [{
    ...
    value: function render() {
      var students = this.data.get('studentsA');
      if (!students) {
        return;
      }
      var list = '';
